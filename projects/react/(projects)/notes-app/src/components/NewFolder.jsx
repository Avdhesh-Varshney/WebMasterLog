...
function NewFolder({ setModalToggle, user }) {

    const { folderId } = useParams()
    const [folderName, setFolderName] = useState("")
    const [error, setError] = useState("")
    const { loading, saveFolder } = useContext(FolderContext);
    ...
    const onSubmit = async (e) => {
        ...
        console.log(user)
        await saveFolder(folderName, folderId || null, user?.id)
        setModalToggle("new", false)
    };
            ...
            <form className="w-full sm:w-2/3 lg:w-1/3 bg-pink-900 p-3 text-white" onSubmit={onSubmit}>
                <h1 className="text-xl mt-1 mb-3 font-medium flex items-center">
                    ...
                    <span
                        ...
                        onClick={() => setModalToggle("new", false)}
                    ><FaTimes /></span>
                </h1>
                <Input
                    label="Folder name"
